...
def is_scandium_gerrit(remote_url):
    p = urllib.parse.urlparse(remote_url)
    return p.hostname == "review.scandiumui.org"
...
def main():
    parser = argparse.ArgumentParser(
        ...
        patches from ScandiumUI's Gerrit instance (or any gerrit instance of your choosing)
    ...
    parser.add_argument(
        "-g",
        "--gerrit",
        default="https://review.scandiumui.org",
        metavar="",
...
def do_git_fetch_pull(args, item):
    ...
    # Try fetching from GitHub first if using scandium gerrit
    if is_scandium_gerrit(args.gerrit):
        if args.verbose:
    ...
    # If not using the scandium gerrit or github failed, fetch from gerrit.
    if args.verbose:
        if is_scandium_gerrit(args.gerrit):
            print(
